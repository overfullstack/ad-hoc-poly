...
class UserRepository(private val client: NamedParameterJdbcTemplate) {
    ...
    fun doesUserExistWith(login: String) =
            client.queryForObject("SELECT 1 FROM users WHERE login = :login LIMIT 1", emptyMap<String, String>(), Int::class.java) == 1
...
class CityRepository(private val client: NamedParameterJdbcTemplate) {
    ...
    fun doesCityExitsWith(name: String) =
            client.queryForObject("SELECT 1 FROM city WHERE name = :name LIMIT 1", emptyMap<String, String>(), Int::class.java) == 1

    fun findAll() = client.queryForList("SELECT * FROM city", emptyMap<String, String>(), City::class.java)

    fun findOne(id: String) =
            client.queryForObject("SELECT * FROM city WHERE name = :name", mapOf("login" to id), City::class.java)
